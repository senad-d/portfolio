...
Background regression coverage lives in `tests/e2e/header-dot-effect.spec.ts` and `tests/e2e/star-black-hole.spec.ts`: immutable source fingerprint (excluding only the additive hook), live gradient/grid values, sparse viewport-sized canvas, twinkle/scroll rendering, keyboard access, local mouse core and recovery, outward star displacement, curved light arcs, continuous stationary-pointer rotation, touch/pen exclusion, reduced motion at load and runtime, and no-JavaScript fallback. These tests guard the source-backed restoration; they do not replace visual comparison with the live assets.
